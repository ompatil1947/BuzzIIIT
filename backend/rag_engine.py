...
class RAGEngine:
    def __init__(self):
        ...
        # Lightweight ONNX embedding function (replaces sentence-transformers/torch)
        print("[RAGEngine] Loading ONNX embedding function...")
        self.embed_fn = embedding_functions.DefaultEmbeddingFunction()
        print("[RAGEngine] Embedding function ready.")
        ...
        # ── Collections (now pass embedding_function so Chroma auto-embeds) ────
    ...
    def _get_or_create_collection(self, name: str):
        try:
            col = self.chroma_client.get_collection(name=name, embedding_function=self.embed_fn)
            print(f"[RAGEngine] Found existing collection '{name}' ({col.count()} docs).")
            return col
        except Exception:
            print(f"[RAGEngine] Creating new collection '{name}'.")
            return self.chroma_client.create_collection(name=name, embedding_function=self.embed_fn)
    ...
    def _embed_all_restaurants(self):
        ...
        # No manual .encode() — Chroma embeds automatically via embedding_function
        self.restaurants_col.add(ids=ids, documents=texts, metadatas=metadatas)
    ...
    # ── Intent extraction (rule-based) — UNCHANGED, keep your existing parse_filters() ──

    def parse_filters(self, query: str) -> Dict[str, Any]:
        # ... exactly as you have it, no changes needed here ...
        ql = query.lower()
    ...
    def process_chat(self, query: str, history: List[Dict[str, str]]):
        ...
        # 2. Semantic search — pass query_texts instead of query_embeddings
        ...
        # 3. Merge restaurant IDs (DB filter + vector search) — UNCHANGED
        ...
        # 5. Build Gemini history — UNCHANGED
        gemini_history = []
        ...
        # 7. Build sources list — UNCHANGED
